...
class Seq2SeqSummarizer(object):
    ...
    def __init__(self, config):
        ...
        encoder_lstm1 = Bidirectional(LSTM(latent_dim,
        ...
        encoder_lstm2 = Bidirectional(LSTM(latent_dim,
                                           return_sequences=True,
                                           return_state=True,
                                           dropout=0.4,
        ...
        encoder_lstm3=Bidirectional(LSTM(latent_dim, 
                                         return_state=True, 
                                         return_sequences=True,
                                         dropout=0.4,
                                         recurrent_dropout=0.4), name='encoder_lstm_layer3')
        ...
        self.encoder_model = Model(inputs=encoder_inputs,outputs=[encoder_outputs, forward_h, forward_c, backward_h, backward_c])

        # Decoder setup
        # Below tensors will hold the states of the previous time step
        decoder_state_input_h = Input(shape=(latent_dim*2,))
        decoder_state_input_c = Input(shape=(latent_dim*2,))
        decoder_hidden_state_input = Input(shape=(self.max_text_len,latent_dim*2))
...
#         self.session.run(init)
    ...
    def summarize(self, input_text):
        ...
        e_out, forward_h, forward_c, backward_h, backward_c = self.encoder_model.predict(transformed_text)
